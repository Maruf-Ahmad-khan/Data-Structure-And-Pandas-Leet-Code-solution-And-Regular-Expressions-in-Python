...
class Pair:
    ...
    def __setitem__(self,i,val):
        ...
        if i<0 or i>1:
            raise IndexError
    def dist(self,otherpair):
        """returns distance between self and otherpair"""
        return sqrt((self[0]-otherpair[0])**2+(self[1]-otherpair[1])**2)
...
class Zooming(Canvas):
    ...
    def canvas2world(self,*canvascoord):
        """takes a canvas coordinate as a tuple and reutrns the world coordinate"""
    ...
    def panbindings(self):
        """arranges for MMB to pan the canvas like typical tk"""
        def press(self,ev):
        ...
        def release(self,ev):
            pass
        self.bind("<ButtonPress-2>",lambda ev: press(self,ev))
        self.bind("<B2-Motion>",lambda ev: motion(self,ev))
        self.bind("<B2-ButtonRelease>",lambda ev: release(self,ev))
        
    def zoombindings(self):
        """arranges for ctrl-MMB to zoom the canvas like rayz did"""
        ...
        def motion(self,ev):
            self.scale(self.firstmouse.x,self.firstmouse.y,
                       1+.02*(ev.x-self.prevmouse.x),
                       1-.02*(ev.y-self.prevmouse.y))
            self.prevmouse=Pair(ev.x,ev.y)
        def release(self,ev):
            pass
        self.bind("<Control-ButtonPress-2>",lambda ev: press(self,ev))
        self.bind("<Control-B2-Motion>",lambda ev: motion(self,ev))
        self.bind("<Control-B2-ButtonRelease>",lambda ev: release(self,ev))
